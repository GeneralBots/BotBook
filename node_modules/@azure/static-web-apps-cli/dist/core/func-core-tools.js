...
export function detectTargetCoreToolsVersion(nodeVersion) {
    // Pick the highest version that is compatible with the specified Node version
    if (nodeVersion >= 18 && nodeVersion <= 20)
        return 4;
...
let installedSystemCoreToolsVersionIsCached = false;
let cachedSystemCoreToolsVersion = undefined;
export function setCachedInstalledSystemCoreToolsVersion(version) {
    cachedSystemCoreToolsVersion = version;
    installedSystemCoreToolsVersionIsCached = true;
}
export function resetCachedInstalledSystemCoreToolsVersion() {
...
export async function getLatestCoreToolsRelease(targetVersion) {
    try {
...
export async function downloadCoreTools(version) {
...
export function removeDownloadedCoreTools(version) {
