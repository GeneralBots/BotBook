import { DATA_API_BUILDER_BINARY_NAME, DATA_API_BUILDER_COMMAND, DATA_API_BUILDER_FOLDER, DATA_API_BUILDER_LATEST_TAG, DATA_API_BUILDER_RELEASE_METADATA_URL, DEFAULT_DATA_API_BUILDER_BINARY, } from "../constants.js";
 ...
 * @returns binaryPath
 */
export async function installAndGetDataApiBuilder() {
...
async function downloadAndUnzipBinary(releaseMetadata, platform) {
    try {
        ...
        if (!fs.existsSync(binaryPath)) {
            ...
            const zipFilePath = await downloadAndValidateBinary(releaseMetadata, DATA_API_BUILDER_BINARY_NAME, DATA_API_BUILDER_FOLDER, releaseMetadata?.versionId, platform);
...
export function getDefaultDataApiBuilderBinaryForOS(platform) {
    switch (platform) {
        case "win-x64":
